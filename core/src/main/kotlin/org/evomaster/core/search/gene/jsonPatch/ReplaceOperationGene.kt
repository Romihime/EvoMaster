 ...
 * Signature: { "op": "replace", "path": "...", "value": ... }
 */
class ReplaceOperationGene(
    pathGene: JsonPointerGene,
    val valueGene: Gene,
    resourceSchema: Gene? = null,
    geneName: String = "replaceOp"
) : JsonPatchOperationGene(
    geneName, pathGene, resourceSchema,
    listOf(pathGene, valueGene)
) {

    override fun operationName() = "replace"

    override fun copyContent(): Gene {
        return ReplaceOperationGene(
    ...
    override fun getValueAsPrintableString(
        ...
        return """{"op":"replace","path":"$pathValue","value":$valueStr}"""
    }

    override fun containsSameValueAs(other: Gene): Boolean {
        if (other !is ReplaceOperationGene) return false
